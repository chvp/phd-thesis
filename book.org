...
**** TODO Hogescholen, lerarenopleidingen contacteren en getuigenis vragen
:PROPERTIES:
...
To work correctly with shared memory, synchronisation primitives have to be used.
...
The functionality for performing synchronous communication with the main thread from a web worker was parceled off into its own library (=sync-message=).
